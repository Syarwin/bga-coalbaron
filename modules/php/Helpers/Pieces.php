...
class Pieces extends DB_Manager
{
    ...
    final static function checkId(&$id, $like = false)
    {
        if (is_null($id)) {
            throw new \BgaVisibleSystemException(
                'Class Pieces: id cannot be null'
            );
        }
        ...
        if (preg_match("/^[A-Za-z_0-9${extra}]+$/", $id) == 0) {
            throw new \BgaVisibleSystemException(
                "Class Pieces: id must be alphanum and underscore non empty string '$id'"
            );
        }
    }
    ...
    final function checkIdArray($arr)
    {
        if (is_null($arr)) {
            throw new \BgaVisibleSystemException(
                'Class Pieces: tokens cannot be null'
            );
        }

        if (!is_array($arr)) {
            throw new \BgaVisibleSystemException(
                'Class Pieces: tokens must be an array'
            );
            foreach ($arr as $id) {
                self::checkId($id);
            }
    ...
    final static function checkState($state, $canBeNull = false)
    {
        if (is_null($state) && !$canBeNull) {
            throw new \BgaVisibleSystemException(
                'Class Pieces: state cannot be null'
            );
        }

        if (!is_null($state) && preg_match('/^-*[0-9]+$/', $state) == 0) {
            throw new \BgaVisibleSystemException(
                'Class Pieces: state must be integer number'
            );
        }
    }
    ...
    final static function checkPosInt($n)
    {
        if ($n && preg_match('/^[0-9]+$/', $n) == 0) {
            throw new \BgaVisibleSystemException(
                'Class Pieces: number of pieces must be integer number'
            );
        }
    }
